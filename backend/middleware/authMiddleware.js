...
const asyncHandler = require('express-async-handler'); // Para manejar errores asíncronos de forma limpia
...
// Middleware para asegurar que solo los usuarios con rol 'user' puedan acceder
// Este es el middleware que faltaba en tu exportación original
const userOnly = (req, res, next) => {
    if (req.user && req.user.role === 'admin') {
        return res.status(403).json({ message: 'Acceso denegado. Los administradores no tienen carrito de compras personal.' });
    }
    next();
};
...
module.exports = {
    protect,
    authorizeRoles,
    userOnly, // ¡Ahora userOnly también se exporta!
};
